...
- `npm install` installs all workspace dependencies. Use npm at the repo root.
...
- `origin` is the project repository for `np-servicerequest.org`.
- `upstream` should continue to point to the local/published template repository used to create this site.
- Prefer pulling structural improvements from the template instead of drifting the repo layout unnecessarily.
