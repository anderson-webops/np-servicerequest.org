...
const siteName = 'NP Service Request'
const frontendKind = 'nuxt'
const frontendPort = Number(process.env.A11Y_FRONTEND_PORT || 3351)
const apiPort = Number(process.env.A11Y_API_PORT || 3051)
...
async function stopProcessTree(child) {
  if (!child.pid || !processIsRunning(child))
    return
  ...
  try {
    ...
    if (error?.code !== 'ESRCH')
      console.warn(`Could not stop frontend process: ${error.message}`)
    return
  }
  ...
  try {
    ...
    if (error?.code !== 'ESRCH')
      console.warn(`Could not force stop frontend process: ${error.message}`)
  }
