...
for (const absent of ['/home/builder', '/mnt/input', '/app/back-end/src', '/app/front-end/src', '/app/.git'])
...
if (process.argv[2] === 'missing-module') {
  ...
  assert.equal(metadata.revision, manifest.commit)
  // Readiness must follow the actual durable data dependency without leaking
  // dependency details from the minimal probe.
  ...
  for (let run = 0; run < 2; run++) {
    const result = spawnSync(process.execPath, [
      '/harness/backend-runtime-smoke.mjs',
      '/app',
      '/harness/post-deploy-smoke.mjs',
    ], {
