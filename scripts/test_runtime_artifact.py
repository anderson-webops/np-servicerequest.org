...
class RuntimeArtifactTests(unittest.TestCase):
    def setUp(self):
        ...
        backend = {"version": "1.0.0", "dependencies": {"express": "5.2.1"}}
        ...
        (self.root / "node_modules/express/package.json").write_text(json.dumps({"version": "5.2.1"}))
        (self.root / "package-lock.json").write_text(json.dumps({
            "version": "1.0.0",
            "packages": {
                "": {"version": "1.0.0"},
                "back-end": backend,
                "front-end": {"version": "1.0.0"},
                "node_modules/express": {"version": "5.2.1"},
            },
        }))
        marker = json.dumps({"version": "1.0.0", "revision": "a"*40, "releasedAt": "2026-09-16T00:00:00Z"})
        (self.root / "front-end/.output/public/release.json").write_text(marker)
        (self.root / ".np-servicerequest-release-prepared.json").write_text(marker)
    ...
    def test_missing_module_even_if_the_inventory_omits_it(self):
        (self.root / "back-end/dist/data.js").unlink()
    ...
    def test_missing_production_dependency(self):
        ...
        lock["packages"]["back-end"] = package
    ...
    def test_malformed_identity_cannot_be_rehashed(self):
        for marker in [{}, {"version": "1.0.0", "revision": "a" * 40},
                       {"version": "1.0.0", "revision": "a" * 40, "releasedAt": "2026-02-30T00:00:00Z"}]:
            with self.subTest(marker=marker):
                for name in ["front-end/.output/public/release.json", ".np-servicerequest-release-prepared.json"]:
    ...
    def test_unlisted_native_and_development_packages_are_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "undeclared native binding"):
            artifact.validate(self.root, self.manifest())
        native.unlink()
        package = self.root / "node_modules/fixture/package.json"
    ...
    def test_exact_archive_roundtrip_and_post_copier_verification(self):
        with tempfile.TemporaryDirectory(dir=self.root.parent) as temporary:
            ...
            (unpacked / "back-end/dist/data.js").unlink()
            ...
            manifest["files"].pop("back-end/dist/data.js")
