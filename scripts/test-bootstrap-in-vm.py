...
marker = Path('/run/np-servicerequest-source-validation-vm')
assert marker.is_file() and marker.read_text() == 'isolated-public-source-fixture\n'
assert marker.stat().st_uid == 0
assert not Path('/srv/np-servicerequest.org').exists(), 'Use a fresh disposable VM, never an installed host'
source = Path(__file__).resolve().parent.parent
control = Path('/root/np-servicerequest-admin-fixture')
...
helper = Path('/usr/local/libexec/np-servicerequest-release') / version
assert (helper / 'deploy/systemd/promote-release.sh').is_file()
assert helper.stat().st_uid == 0 and not helper.stat().st_mode & 0o022
unit = Path('/etc/systemd/system/np-servicerequest.service')
original_unit = unit.read_bytes()
assert subprocess.run(['systemctl', 'is-active', '--quiet', 'np-servicerequest.service']).returncode != 0
base = Path('/srv/np-servicerequest.org')
for path in [base / 'current', base / 'releases/unauthorized']:
    denied = subprocess.run(['runuser', '-u', 'np-servicerequest', '--', 'touch', str(path)], capture_output=True)
...
assert not Path('/usr/local/libexec/np-servicerequest-release/999.0.0').exists()
base.chmod(0o750)
...
sink = Path('/root/np-servicerequest-cache-fixture')
sink.mkdir(mode=0o755)
cache = base / 'shared/npm-cache'
cache.symlink_to(sink)
old = subprocess.run(['/usr/bin/install', '-d', '-o', 'np-servicerequest', '-g', 'np-servicerequest', '-m', '0700', str(cache)], capture_output=True)
...
(control / 'package.json').write_text(json.dumps(package))
(control / 'deploy/systemd/np-servicerequest.service').chmod(0o666)
assert run().returncode != 0
assert not Path('/usr/local/libexec/np-servicerequest-release/999.0.2').exists()
assert unit.read_bytes() == original_unit
