...
if name == 'curl':
    ...
    if mode in ['alternate-port','custom-probes'] and '127.0.0.1:3016' in url:sys.exit(22)
...
def setup(root):
    ...
    (control/'package.json').write_text(json.dumps({'version':'1.2.43'}))
    ...
    backend={'version':'1.0.0','type':'module','dependencies':{'express':'5.2.1'}}
    frontend={'version':'2.0.0','type':'module'}
    express=candidate/'node_modules/express/package.json'
    express.parent.mkdir(parents=True, exist_ok=True)
    express.write_text(json.dumps({'version':'5.2.1'}))
    for name,value in [('package.json',package),('front-end/package.json',frontend),('back-end/package.json',backend),
                       ('package-lock.json',{'version':'1.2.43','packages':{'':package,'back-end':backend,'front-end':frontend,'node_modules/express':{'version':'5.2.1'}}})]:
        (candidate/name).write_text(json.dumps(value))
    metadata={'version':'1.2.43','revision':'a'*40,'releasedAt':'2026-09-20T00:00:00Z'}
    for name in ['.np-servicerequest-release-prepared.json','front-end/.output/public/release.json']:
    ...
    metadata.update(version='1.2.42',revision='b'*40)
    for name in ['.np-servicerequest-release-prepared.json','front-end/.output/public/release.json']:
...
for mode in modes:
    ...
    if mode=='previous-is-parent':
        ...
        shutil.copyfile(previous/'.np-servicerequest-release-prepared.json',root/'releases/.np-servicerequest-release-prepared.json')
        previous=root/'releases'
    if mode=='invalid-previous-identity':(previous/'.np-servicerequest-release-prepared.json').write_text('{}')
    held=None
    if mode=='lock-contention':
        held=(recovery/'promotion.lock').open('w');fcntl.flock(held,fcntl.LOCK_EX|fcntl.LOCK_NB)
    env={**os.environ,'NODE_BIN_DIR':'/fixture/runtime','PUBLIC_HOST':'np-servicerequest.org',
    ...
    if mode=='mismatched-readiness-origin':env['READINESS_URL']='http://127.0.0.1:3016/readyz'
    ...
    if success:
        ...
        if mode in ['alternate-port','custom-probes']:assert '127.0.0.1:3016' not in probes
